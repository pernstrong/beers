...
jest.mock('../../apiCalls')
 ...
 describe('App', () => {
    ...
    it('should let user search for breweries', async () => {
        ...
        fireEvent.change(getByPlaceholderText('search by zip code'), { target: {value: '80220'}})
    ...
    it('should let a user add a rating to the brewery', async () => {
        ...
        fireEvent.change(getAllByDisplayValue('')[0], { target: { value: '4'}})

        expect(getByText('My Rating: 4')).toBeInTheDocument()
    })

    it('should let a user add notes about beers for the brewery', async () => {
        const { getByText, getAllByText, getByPlaceholderText, getByDisplayValue } = render(<MemoryRouter><App /></MemoryRouter>)
    ...
    it('should allow a user to favorite a brewery', async () => {
        ...
        fireEvent.click(getByText('My Favorites'))
